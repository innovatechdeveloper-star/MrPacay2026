...
console.log('🎨 CONVERTIDOR PNG → ZPL ^GFA - LOGO MISTI\n');
console.log('═══════════════════════════════════════════════════\n');

const LOGO_PATH = './founds/godex/logo-mistiv2.png';
const OUTPUT_JS = './logo-misti-zpl-generado.js';
...
async function convertirPngAZpl() {
    try {
        ...
        const jsContent = `// Logo MISTI generado automáticamente
// Fecha: ${new Date().toLocaleDateString('es-PE')}
// Tamaño: ${width}×${height}px (${(width/11.81).toFixed(1)}mm × ${(height/11.81).toFixed(1)}mm a 300 DPI)

const LOGO_MISTI_ZPL = \`${zplCommand}\`;

module.exports = { LOGO_MISTI_ZPL };

// Uso en ZPL:
// ^FO177,305    <- Posición X=177 (columna derecha), Y=305
// ${zplCommand}
`;
        ...
        console.log(`\n💡 Copia la constante LOGO_MISTI_ZPL al archivo server.js\n`);
